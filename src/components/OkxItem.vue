<script setup lang="ts">
import { ref, onMounted, watchEffect, nextTick, type Ref } from 'vue'
import { event } from '../utils/event-bus'

const toEl:Ref<HTMLUListElement | undefined>  = ref()
const itemShow = ref(false)

const ua = navigator.userAgent;
const isIOS = /iphone|ipad|ipod|ios/i.test(ua);
const isAndroid = /android|XiaoMi|MiuiBrowser/i.test(ua);
const isMobile = isIOS || isAndroid;
const isOKApp = /OKApp/i.test(ua);

function goOkx() {
  if(isMobile && !isOKApp){
    const encodedUrl = "https://www.okx.com/download?deeplink=" + encodeURIComponent("okx://wallet/dapp/url?dappUrl=" + encodeURIComponent(location.href));
    window.location.href = encodedUrl;
  } else {
    window.open('https://www.okx.com/cn/web3')
  }
}

watchEffect(() => {
  if(toEl.value) {
    nextTick(() => {
      itemShow.value = true
    })
  } else {
    itemShow.value = false
  }
})

// 判断是否是okx
function isOkrItem(item: HTMLElement):boolean {
  const pEl = item.querySelector('p')
  if(pEl?.innerText.includes('OKX Wallet')) {
    return true
  } else {
    return false
  }
}

onMounted(() => {
  event.on('wallet-select', (el: any) => {
    console.log('wallet-select', el)
    if(!el) return
    const allLi = document.querySelectorAll('.swv-modal-list li')
    const allliElments = [...allLi] as HTMLElement[]
    const hasOkx = allliElments.find(item => {
      return isOkrItem(item)
    })
    console.log('wallet-select', el)
    toEl.value = !hasOkx ? el : undefined
  })
})
</script>

<template>
  <Teleport :to="toEl" v-if="toEl">
    <li>
      <button class="swv-button" @click="goOkx">
        <i class="swv-button-icon">
          <img src="data:image/png;base64," alt="OKX Wallet icon">
        </i>
        <p>OKX Wallet</p>
        <div class="swv-wallet-status"></div>
      </button>
    </li>
  </Teleport>
</template>

<style scoped>
</style>
